...
version(/+$DIDE_REGION+/all) {
	...
	class Camera
	{
		...
		*/
		...
		/+
			Assistant: 
			/+H1: Camera Class Cheat Sheet+/
			
			/+H2: Properties+/
			/+Italic:  +//+Highlighted: isPerspective: bool+//+Italic:  - Perspective (true) or orthographic (false) projection+/
			/+Italic:  +//+Highlighted: verticalFov: float+//+Italic:  - Vertical field of view (30-140 degrees)+/
			/+Italic:  +//+Highlighted: verticalVisibleUnits: float+//+Italic:  - Visible height in orthographic mode+/
			/+Italic:  +//+Highlighted: near: float+//+Italic:  - Near clipping plane (10 default)+/
			/+Italic:  +//+Highlighted: far: float+//+Italic:  - Far clipping plane (10000 default)+/
			/+Italic:  +//+Highlighted: unit: string+//+Italic:  - Measurement unit label+/
			/+Italic:  +//+Highlighted: origin: vec3+//+Italic:  - Camera position (get/set)+/
			/+Italic:  +//+Highlighted: pivot: vec3+//+Italic:  - Rotation center point+/
			
			/+H2: Direction Vectors+/
			/+Italic:  +//+Highlighted: right(): vec3+//+Italic:  - Right direction vector+/
			/+Italic:  +//+Highlighted: up(): vec3+//+Italic:  - Up direction vector  +/
			/+Italic:  +//+Highlighted: forward(): vec3+//+Italic:  - Forward direction vector+/
			
			/+H2: Core Operations+/
			/+Italic:  +//+Highlighted: reset()+//+Italic:  - Reset to default position/orientation+/
			/+Italic:  +//+Highlighted: synch()+//+Italic:  - Synchronize camera parameters after changes+/
			/+Italic:  +//+Highlighted: dump()+//+Italic:  - Output camera state as JSON+/
			
			/+H2: Camera Controls+/
			/+Italic:  +//+Highlighted: pan(vec2 delta, float viewHeight, float speed=1)+//+Italic:  - Move camera laterally+/
			/+Italic:  +//+Highlighted: look(vec2 delta)+//+Italic:  - Rotate camera view+/
			/+Italic:  +//+Highlighted: zoom(float amount, Cursor3D cursor)+//+Italic:  - Zoom in/out+/
			/+Italic:  +//+Highlighted: rotateAroundPivot(vec2/vec3 angles)+//+Italic:  - Rotate around pivot point+/
			/+Italic:  +//+Highlighted: adjustFovY(float degIncrement)+//+Italic:  - Adjust field of view+/
			
			/+H2: Navigation+/
			/+Italic:  +//+Highlighted: navigate(bool keys, bool mouse, float height, Cursor3D cursor)+//+Italic:  - Integrated navigation handling+/
				 /+Italic:  MMB: Pan camera+/
				 /+Italic:  RMB: Rotate camera (Ctrl: look, Shift: roll)+/
				 /+Italic:  Mouse wheel: Zoom+/
				 /+Italic:  Home key: Reset camera+/
			
			/+H2: Projection Setup+/
			/+Italic:  +//+Highlighted: setupCameraMatrices(width, height, subRect, x0, y0, x1, y1, out mView, out mProjection)+/
			/+Italic:  - Get view/projection matrices+/
			
			/+H2: Utility Functions+/
			/+Italic:  +//+Highlighted: pivotDistance(): float+//+Italic:  - Distance to pivot point+/
			/+Italic:  +//+Highlighted: pivotPlane(): Plane+//+Italic:  - Plane through pivot perpendicular to view+/
			/+Italic:  +//+Highlighted: updatePivot(Cursor3D cursor)+//+Italic:  - Update pivot based on cursor position+/
			/+Italic:  +//+Highlighted: lookFrom(vec3 target, vec3 dir, float dist)+//+Italic:  - Position camera looking at target+/
			
			/+H2: Coordinate System+/
			/+Italic:  Right-handed coordinate system+/
			/+Italic:  Y-up orientation+/
			/+Italic:  GLSL-like vector/matrix operations supported+/
			
			/+Note: Usage(prompt_hit: 128, prompt_miss: 3966, completion: 522, HUF: 0.62, price: 100%)+/
		+/
	}; 
...
version(/+$DIDE_REGION 3DS model+/all)
{
	...
	class Material
	{
		...
		override string toString() const
		{
			...
			//
				return "Material(%s, amb:%s, diff:%s, spec:%s, shin:%s, bumpa:%s, maps:%s)"
				.format(name, ambient, diffuse, specular, shininess, bumpAmount, [texMap, opacMap, bumpMap, specMap, reflMap]); 
