...
version(/+$DIDE_REGION+/all)
{
	...
	struct ClickDetector
	{
		...
		void update(bool state, DateTime t=DateTime.init)
		{
			...
			if(pressed) {
				...
				if(longPressing.chkClear) { longClicked = clicked = true; }
	...
	class InputManager
	{
		...
		void pressKey(ubyte vk, bool press=true)
		{
			...
			{
				/+
					Note: keybd_event, SendInput: multimedia keys that launch apps aren't working. 
		...
		bool repeatLogic(
			in bool active, in bool pressed, ref Time repeatNextTime,
			in Time delay1, in Time delay2
		)
		{
			if(active)
			{
	...
	private
	{
		...
		interface IDirectInput8A : IUnknown
		{
			...
			/*
				...
				function EnumDevicesBySemantics(ptszUserName: PAnsiChar; const lpdiActionFormat: TDIActionFormatA; 
					lpCallback: TDIEnumDevicesBySemanticsCallbackA; pvRef: Pointer; dwFlags: DWORD): HResult; stdcall;
