...
class OpenCL_error:Exception
{
		//Todo: update this with line info
	this(string s)
	{ super(s); }
}
...
{ align(1): uint image_channel_order, image_channel_data_type; }
struct cl_buffer_region
{ align(1): size_t origin, size; }
...
enum 
{
	...
	CL_CONTEXT_PLATFORM_INFO  = 0x1084,
} //CL_CONTEXT_PLATFORM

...
private string clErrorStr(int e)
{
		//Todo: erre csinalni egy automata formazot
	switch(e)
	{
...
class CLFuncts
{
		...
		extern(Windows)
	{
		...
		int function(
			const cl_command_queue, const cl_kernel, size_t work_dim, const size_t* global_ofs, const size_t* global_size, const size_t* local_size,
							 size_t num_events_in_wait_list, const cl_event* event_wait_list, cl_event* event
		...
		void* function(
			const cl_command_queue queue, const cl_mem buffer, cl_bool blocking_map, cl_map_flags,
				  size_t offset, size_t cb, size_t num_events_in_wait_list, const cl_event* event_wait_list, cl_event* event, int* errcode_ret
		) clEnqueueMapBuffer;
		
		int function(
			const cl_command_queue, const cl_mem buffer, cl_bool blocking_write,
				  size_t offset, size_t cb, const void* ptr, size_t num_events_in_wait_list, const cl_event* event_wait_list, cl_event* event
		...
		static void enforce(bool b, lazy string s, string file = __FILE__, int line = __LINE__)
	{
		if(!b)
		throw new Exception("OpenCL error: "~s, file, line);
	}
		...
		void loadFuncts() //must be called right after it got an active opengl contect
	{
		...
		mixin(
			[FieldNameTuple!CLFuncts].filter!(x => x.startsWith("cl"))
											   .map!(x => `GPA(`~x~`,"`~x~`");` )
											   .join
		);
	}
		...
		CLDevice[] devices()
	{
		if(chkSet(initialized))
		{
			foreach(plt; getPlatformIDs(1))
			{
				...
				if(!devs.empty)
				{
						//got the gpu category
					foreach(idx, id; devs)
					devices_ ~= new CLDevice(idx.to!int, id);
	...
	{
		if(err)
		throw new Exception(clErrorStr(err), file, line);
	}
		...
		cl_device_id[] getDeviceIDs(cl_platform_id platform, cl_device_type device_type)
	{
		...
		if(n)
		clChk(clGetDeviceIDs(platform, device_type, n, &devs[0], &n));
		return devs;
	}
	
		void releaseDevice(ref cl_device_id d)
	{ clChk(clReleaseDevice(d));  d = null; }
	
		auto getDeviceInfo_int (const cl_device_id id, int name)
	{ int        i; clChk(clGetDeviceInfo(id, name, i.sizeof, &i, null)); return i     ; }
		auto getDeviceInfo_long(const cl_device_id id, int name)
	{ long       L; clChk(clGetDeviceInfo(id, name, L.sizeof, &L, null)); return L     ; }
		auto getDeviceInfo_str (const cl_device_id id, int name)
	{ char[4096] s; clChk(clGetDeviceInfo(id, name, s.sizeof, &s, null)); return (&s[0]).toStr; }
		...
		auto createContext(const cl_device_id[] devices)
	{
		...
		void releaseContext(ref cl_context ctx)
	{ clChk(clReleaseContext(ctx));  ctx = null; }
		...
		auto createCommandQueue(const cl_context ctx, const cl_device_id id, cl_command_queue_properties props=0)
	{
		...
		void releaseCommandQueue(ref cl_command_queue que)
	{ clChk(clReleaseCommandQueue(que)); }
		...
		string getProgramInfo_binary(const cl_program prg)
	{
		...
		void releaseProgram(ref cl_program p)
	{ clChk(clReleaseProgram(p));  p = null; }
		...
		cl_event enqueueNDRangeKernel(const cl_command_queue queue, const cl_kernel kernel, const size_t[] global_ofs, const size_t[] global_size, const size_t[] local_size, const cl_event[] event_wait_list=[])
	{
		...
		const cl_command_queue queue, const cl_kernel kernel, size_t global_ofs, size_t global_size, size_t local_size,
										const cl_event[] event_wait_list=[]
	)
	{ return enqueueNDRangeKernel(queue, kernel, [global_ofs], [global_size], [local_size], event_wait_list); }
	
		void releaseKernel(ref cl_kernel k)
	{ clChk(clReleaseKernel(k));  k = null; }
		...
		auto createBuffer(const cl_context ctx, int flags, size_t size, void* host_ptr=null)
	{
		...
		const cl_command_queue queue, const cl_mem buffer, cl_bool blocking_map, cl_map_flags map_flags, size_t offset, size_t cb,
	...
	)
	{
		...
		const cl_command_queue queue, const cl_mem buffer, cl_bool blocking_write, size_t offset, size_t cb, const void* ptr,
								  const cl_event[] event_wait_list=[], cl_event* event=null
	)
	{ clChk(clEnqueueWriteBuffer(queue, buffer, blocking_write, offset, cb, ptr, event_wait_list.length, event_wait_list.ptr, event)); }
	
		void enqueueUnmapMemObject(
		const cl_command_queue queue, const cl_mem buffer, void* mapped_ptr,
	...
	{ clChk(clEnqueueUnmapMemObject(queue, buffer, mapped_ptr, event_wait_list.length, event_wait_list.ptr, event)); }
		...
		void releaseMemObject(ref cl_mem m)
	{ clChk(clReleaseMemObject(m));  m = null; }
	
		void finish(cl_command_queue que)
	{ clChk(clFinish(que)); }
		void flush (cl_command_queue que)
	{ clChk(clFlush (que)); }
		...
		static void _test()
	{
		...
		with(kernel3.run(0, workCount, buf1, buf2))
		{ waitFor; elapsed_sec.writeln; }
...
class CLDeviceInfo
{
	...
	this(CLDevice device)
	{
		this.device = device;
		auto id()const
		{ return cast(cl_device_id)device.id; }
		
		auto s(int n)
		{ return cl.getDeviceInfo_str (id, n); }
		auto i(int n)
		{ return cl.getDeviceInfo_int (id, n); }
		auto L(int n)
		{ return cl.getDeviceInfo_long(id, n); }
	...
	{ return format("Idx:%d ID:%x Device: %s  %.2f TFlops  Core:%d MHz  Strm:%d  MEM:%d(%s) MB", idx, vendorID, deviceName, TFlops, coreMHz, streams, memoryMB, maxMemAllocMB); }
}
...
class CLDevice
{
	...
	{ return cast(void*)fcontext !is null; }
	...
	{ active = false; }
		...
		private void _removeObject(CLObject o)
	{ objects = objects.remove(objects.countUntil(o)); }
	...
	{ return new CLKernel(this, src, kernelName, file, line); }
		auto newBuffer(size_t size, string flags="rw")
	{ return new CLBuffer(this, size, flags); }
...
class CLBuffer:CLObject
{
	...
	{ return fsize; }
	@property auto flags() const
	{ return fflags; }
	@property auto mem  () const
	{ return fmem ; }
	...
	{
		if(fsize==newSize && flags==fflagsStr)
		return;
		unmap;
		cl.releaseMemObject(fmem); fmem = null;
	...
	void* map(string rw="rw")
	{
		...
		if(fmap &&((fl & fmapFlags)==fl))
		return fmap; //got it already mapped for the same or more rw rights
		unmap;
		fmapFlags = fl;
	...
	T[] array(T)()
	{ return (cast(T*)map)[0..size/T.sizeof]; }
	auto chars	()
	{ return array!char ; }
	auto bytes	()
	{ return array!byte ; }
	...
	private void enforceRange(size_t n)
	{ cl.enforce(n<=size, "CLBuffer out of range"); }
...
class CLKernel:CLObject
{
	...
	private this(CLDevice device, string src, string kernelName, string srcFile=__FILE__, int srcLine=__LINE__)
	{
		...
		if(src.startsWith("\x7FELF"))
		{
				//ELF binary image
			program = cl.createProgramWithBinary(device.context, [device.id], src);
		}else if(src.indexOf(".amdcl2")>=0 || src.indexOf(".amdcl12")>=0 && src.indexOf(".kernel")>=0)
		{
				//GCN assembly
			try
			{ fBinary = assemble(src); }catch(Exception e)
			{ throwCompilationError(e, src, srcFile, srcLine); }
			program = cl.createProgramWithBinary(device.context, [device.id], fBinary);
		}else if(src.indexOf("__kernel")>=0)
		{
				//OpenCL source
			try
			{ program = cl.createProgramWithSource(device.context, src); }catch(Exception e)
			{ throwCompilationError(e, src, srcFile, srcLine); }
		}else
		cl.enforce(0, "CLKernel: Unknown kernel type");
	...
	string binary()
	{
			//Todo: ez egy cached value. Erre kene csinalni egy templatet.
		if(fBinary.empty)
		fBinary = cl.getProgramInfo_binary(program);
		return fBinary;
	}
	...
	private void throwCompilationError(Exception e, string src, string srcFile, int srcLine)
	{
		...
		{
			auto chk(string s1, string s2)
			{ return isWild!(false, '\x01', '\x02')(s1, s2); }
			int[] idx1List;
			foreach(i, s1; src1)
			if(chk(s1, src2[idx2]))
			idx1List ~= i.to!int;
			...
			int scoreOf(int idx1, int idx2)
			{
				...
				bool doit(int i)
				{
					if(idx2+i<0 || idx2+i>=src2.length)
					return false;
					if(idx1+i<0 || idx1+i>=src1.length)
					return false;
					if(!chk(src1[idx1+i], src2[idx2+i]))
					return false;
					res++;
					return true;
				}
		...
		string processErrorLine(string s)
		{
			...
			if(s.startsWith(prefix))
			{
				...
				try
				{ s.formattedRead!"%d:%d: Error: %s"(line, col, err); }catch(Throwable)
				{ return ret; }
...
class GcnRegPool
{
		...
		struct Reg
	{
		...
		auto toString ()const
		{ return `%-4s:%s`.format(realName, aliasName); }
		
		auto realName ()const
		{ return type~idx.text; }
		auto aliasName()const
		{ return allocatedName~(isArray ? "["~allocatedIdx.text~"]" : ""); }
		
		bool allocated()const
		{ return !allocatedName.empty; }
	...
	{
		if(regs.any!(r => r.allocatedName==name))
		return false;
		...
		void alloc(char type, string name, int count=1, int align_=1)
	{
		...
		{
			if(r.idx != r.idx/align_*align_)
			continue;
			if(tryAlloc(type, r.idx, name, count))
			return;
		}
		...
		foreach(s; parts)
		{
			      ...
			      if(s.isWild("?*[?*]"))
			{
				...
				count = wild[1].strip.to!int;
			      ...
			      if(dst>=0)
			alloc(type, dst, s, count);
			else alloc(type, s, count, align_);
		}
	}
	
		void allocS(int idx, string name, int count=1)
	{ alloc('s', idx, name, count); }
		void allocV(int idx, string name, int count=1)
	{ alloc('v', idx, name, count); }
		void allocS(string name, int count=1, int align_=1)
	{ alloc('s', name, count, align_); }
		void allocV(string name, int count=1, int align_=1)
	{ alloc('v', name, count, align_); }
	
		void free(string filter)
	{
		foreach(ref r; regs)
		if(r.allocatedName.isWild(filter))
		r.free;
	}
	...
	{
		int cnt = 0;
		foreach(const r; regs)
		if(r.allocated && r.type==type)
		cnt = r.idx+1;
		return cnt;
	}
	...
	{ return getRegCnt('v'); }
		int sTotal()const
	{ return getRegTotal('s'); }
		int vTotal()const
	{ return getRegTotal('v'); }
...
//Todo: DIDE: Nem birja a .macro-kat a q{} tokenstring.
...
private
{
	...
	GCNString gcnToken(string what, string payload)
	{ return cast(GCNString)(gcnTokenMarkerBegin ~ what ~ gcnTokenMarkerEnd ~ payload); }
}
...
GCNString gcnParams(T)()
{
	...
	void doSLoad(string name, string sName, size_t ofs, size_t size, bool doLoad)
	{
		...
		int calcAlign(size_t s)
		{ return 1<<(s.log2.iFloor); }
		
		sAlloc ~= `%s[%d] align %d`.format(sName, size/4, calcAlign(size/4));
		int dwIdx;
		while(size>0)
		{
			foreach(dw; [16, 8, 4, 2, 1])
			if(size>=dw*4)
			{
	...
	return gcnConcat(
		gcnToken("AllocS", sAlloc.join(';'))                     ,
																					gcnToken("LoadParams", (ofsDecls~loadScript).join('\n')) 
	);
}

GCNString gcnAllocS(string s)
{ return gcnToken("AllocS", s); }
GCNString gcnAllocV(string s)
{ return gcnToken("AllocV", s); }
GCNString gcnCode  (string s)
{ return gcnToken("Code"  , s); }
...
GCNString gcnRoutine(string name, string code, int level=1)
{
	...
	{ return s.replace("#", level.text).replace("@", name).indent(0); }
	
	return gcnConcat(
		gcnToken("Routine", level.text ~ ";" ~ name) ,
																					gcnAllocS("s" ~ name ~ "[2] align 2")        ,
																					gcnCode(prep(header) ~ code ~ prep(footer))  
	);
}

GCNString gcnRoutine1(string name, string code)
{ return gcnRoutine(name, code, 1); }
GCNString gcnRoutine2(string name, string code)
{ return gcnRoutine(name, code, 2); }
GCNString gcnRoutine3(string name, string code)
{ return gcnRoutine(name, code, 3); }
GCNString gcnRoutine4(string name, string code)
{ return gcnRoutine(name, code, 4); }

string gcnMake(GCNString[] parts_...)
{
		void onlyOnce(string s, lazy string name)
	{ enforce(s.empty, "gcnMake: "~name~" already defined."); }
	...
	if(!p.empty)
	{
		...
		if(header.isWild("*.sgprsnum *\r*"))
	sCnt = wild[1].to!int;
		if(header.isWild("*.vgprsnum *\r*"))
	vCnt = wild[1].to!int;
		...
		int[] buffers;
		foreach(int i; 0..8)
	if(header.canFind(".arg buf"~i.text))
	{
		buffers ~= i;
		if(!bufCnt)
