...
version(/+$DIDE_REGION Global System stuff+/all)
{
	version(/+$DIDE_REGION+/all) {
		...
		version(/+$DIDE_REGION Imports+/all)
		{
			...
			public import core.interpolation: InterpolationHeader, InterpolatedLiteral, InterpolatedExpression, InterpolationFooter; 
	...
	}struct application
	{
		...
		__gshared static public
		{
			...
			Time 	deltaTime 	= 0*second,
				appTime 	= 0*second/+It's iTIme in ShaderToy+/; 
			string timeZoneName; 
...
}version(/+$DIDE_REGION Meta programming       +/all)
{
	...
	deprecated string processGenericArgs(string code)
	{
	...
	size_t estimateTotalSize(T)(auto ref T value)
	{
		/+
			Generated by Chrome AI
			/+
				User: dlang how to calculate a size of a struct recursively? 
				If it is a class or has member classes or pointers, count only the pointer, for arrays it should step in.
			+/
			/+User: please generate more complex test inputs!+/
			/+
				260828: 1 perc alatt remeknek kinézô válasz után 1 óra átírás, debuggolás, anyázás.
				A végeredmény így egy közepesen gyors gányolás lett...
			+/
		+/
		
		static if(is(T == class))
		{
			if(value is null) return T.sizeof; 
			static if(__traits(compiles, { size_t s = value.estimateTotalSize_impl; }))
			return value.estimateTotalSize_impl; 
			return T.sizeof + typeid(value).m_init.length; 
		}
		else static if(isDynamicArray!T)
		{
			return T.sizeof + ElementType!T.sizeof * value.length; //don't step in
		}
		else static if(isArray!T)
		{
			return T.sizeof; //don't step in, that'd be a recursive waist of time.
		}
		else static if(is(T == struct))
		{
			static if(__traits(compiles, { size_t s = value.estimateTotalSize_impl; }))
			...
			{ total += 	estimateTotalSize(__traits(getMember, value, fieldName)); }
			return total; 
		}
		else static if(isPointer!T || is(T == interface))
		{ return T.sizeof; }
		else
		{
			// 4. Fallback for primitives (ints, floats, bools, etc.)
			return T.sizeof; 
		}
	} 
	
	void testEstimateTotalSize()
	{
		...
		static struct InnerPayload
		{
			align(1): double precisionVal; 	// 8 bytes
			int[] runtimeChunks; 	// 16 bytes overhead + dynamic content
		} 
		static struct LayerTwo
		{
			align(1): char typeCode; 	// 1 byte
			InnerPayload[2] fixArray; 	// Static array (2 * InnerPayload.sizeof)
			NetworkNode activeNode; 	// 8 bytes pointer (Ignore heap payload)
		} 
		
		{
			auto n = new NetworkNode; 
			enforce(n.estimateTotalSize == 44); 
			enforce(__traits(classInstanceSize, NetworkNode)+NetworkNode.sizeof); 
			
			/+
				static immutable InnerPayload a = {123, [1]}; 
				enforce(a.estimateTotalSize == 28); 
				
				static immutable InnerPayload b = {123, [1,2,3,4]}; 
				enforce(b.estimateTotalSize == 40); 
				auto c = LayerTwo('x', [cast()a, cast()b], n); 
				enforce(c.estimateTotalSize == 93); //its short, because it doesn't step in fixArray
			+/
		}
	} 
	...
	version(/+$DIDE_REGION+/all)
	{
		...
		auto 檢(A...)(ulong location, A args)
		{
			...
			if(dbg.isActive)
			{
				...
				{
					string txt; foreach(i, a; args) { if(i) txt ~= ' '; txt ~= a.text; }
					if(txt.length<=1024)
		...
		auto 同(string T, string sym, string id)()
		{
			...
			/+
				TestPad:
				/+
					Code: mixin(同!(q{float/+w=6 h=1 min=0 max=12 sameBk=1 rulerSides=3 rulerDiv0=11+/},q{val},q{0x1B49859F156A1})); 
					/+
						Changes after the fix:
						/+
							Code: //Invalid:
							auto x = mixin(同!(q{float/+w=6 h=1 min=0 max=12 sameBk=1 rulerSides=3 rulerDiv0=11+/},q{val},q{0x1B56059F156A1})); 
							//Grouping by comma expressions also broken:
							mixin(同!(q{float/+w=6 h=1 min=0 max=12 sameBk=1 rulerSides=3 rulerDiv0=11+/},q{val1},q{0x1B60A59F156A1})),
							mixin(同!(q{float/+w=6 h=1 min=0 max=12 sameBk=1 rulerSides=3 rulerDiv0=11+/},q{val2},q{0x1B67F59F156A1})); 
						+/
					+/
				+/
			+/
		...
		enum 位         (string FILE=__FILE__, size_t LINE=__LINE__) = _LOCATION_!(FILE, LINE); 
		...
		template 碼(LOCATION_t LOCATION, Args...)
		{
			size_t ctfeWriteIES(Args...)(Args args, size_t h)
			{
				static size_t doit(A)(A a, size_t h)
				{
					...
					else static if(is(T==InterpolatedLiteral   !lit , string lit ))	{ wr("\3"); wr(lit); }
					else static if(is(T==InterpolatedExpression!expr, string expr))	{ wr("\4"); wr(expr); }
					else	{ wr("\5"); wr(a.text); }
					return h; 
				} 
...
version(/+$DIDE_REGION Colors+/all)
{
	...
	}version(/+$DIDE_REGION Color constants+/all)
	{
		...
		}version(/+$DIDE_REGION standard ega palette+/all)
		{
			immutable
			clEgaBlack	= (RGB(0x000000)),
			clEgaLowBlue	= (RGB(0xAA0000)),
			clEgaLowGreen	= (RGB(0x00AA00)),
			clEgaLowCyan	= (RGB(0xAAAA00)),
			clEgaLowRed	= (RGB(0x0000AA)),
			clEgaLowMagenta	= (RGB(0xAA00AA)),
			clEgaBrown	= (RGB(0x0055AA)),
			clEgaLightGray	= (RGB(0xAAAAAA)),
			clEgaDarkGray	= (RGB(0x555555)),
			clEgaHighBlue	= (RGB(0xFF5555)),
			clEgaHighGreen	= (RGB(0x55FF55)),
			clEgaHighCyan	= (RGB(0xFFFF55)),
			clEgaHighRed	= (RGB(0x5555FF)),
			clEgaHighMagenta	= (RGB(0xFF55FF)),
			clEgaYellow	= (RGB(0x55FFFF)),
			clEgaWhite	= (RGB(0xFFFFFF)); 
		}
		
		version(/+$DIDE_REGION PET palette+/all)
		{
			immutable
			clPetBlack	= (RGB(0x000000)),
			clPetWhite	= (RGB(0xFFFFFF)),
			clPetRed	= (RGB(0x354374)),
			clPetCyan	= (RGB(0xBAAC7C)),
			clPetPurple	= (RGB(0x90487B)),
			clPetGreen	= (RGB(0x4F9764)),
			clPetBlue	= (RGB(0x853240)),
			clPetYellow	= (RGB(0x7ACDBF)),
			clPetOrange	= (RGB(0x2F5B7B)),
			clPetBrown	= (RGB(0x00454f)),
			clPetPink	= (RGB(0x6572a3)),
			clPetDGrey	= (RGB(0x505050)),
			clPetGrey	= (RGB(0x787878)),
			clPetLGreen	= (RGB(0x8ed7a4)),
			clPetLBlue	= (RGB(0xbd6a78)),
			clPetLGrey	= (RGB(0x9f9f9f)); 
		}version(/+$DIDE_REGION C64 palette+/all)
		{
			immutable 
			clC64Black	= (RGB(  0,   0,   0)),
			clC64White	= (RGB(255, 255, 255)),
			clC64Red	= (RGB(136,   0,   0)),
			clC64Cyan	= (RGB(170, 255, 238)),
			clC64Purple	= (RGB(204,  68, 204)),
			clC64Green	= (RGB(  0, 204, 85)),
			clC64Blue	= (RGB(  0,   0, 170)),
			clC64Yellow	= (RGB(238, 238, 119)),
			clC64Orange	= (RGB(221, 136,  85)),
			clC64Brown	= (RGB(102,  68,   0)),
			clC64Pink	= (RGB(255, 119, 119)),
			clC64DGrey	= (RGB( 51,  51,  51)),
			clC64Grey	= (RGB(119, 119, 119)),
			clC64LGreen	= (RGB(170, 255, 102)),
			clC64LBlue	= (RGB(  0, 136, 255)),
			clC64LGrey	= (RGB(187, 187, 187)); 
		}
		...
		immutable RGB[]
			...
			clEga	= [
			clEgaBlack, clEgaLowBlue, clEgaLowGreen, clEgaLowCyan, clEgaLowRed,
			clEgaLowMagenta, clEgaBrown, clEgaLightGray, clEgaDarkGray, clEgaHighBlue,
			clEgaHighGreen, clEgaHighCyan, clEgaHighRed, clEgaHighMagenta, clEgaYellow,
			clEgaWhite
		],
			clPet	= [
			clPetBlack, clPetWhite, clPetRed, clPetCyan, clPetPurple, clPetGreen, clPetBlue,
			clPetYellow, clPetOrange, clPetBrown, clPetPink, clPetDGrey, clPetGrey,
			clPetLGreen, clPetLBlue, clPetLGrey
		],
			clC64	= [
