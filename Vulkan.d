...
version(/+$DIDE_REGION Vulkan classes+/all)
{
	version(/+$DIDE_REGION+/all)
	{
		...
		static GEN_ShaderLayoutDeclarations(in 表 table)
		{
			...
			foreach(row; table.rows)
			{
				foreach(dir; 0..2)
				{
					const stage = row[dir], dirStr = ((dir)?(q{in}):(q{out})); 
					if(stage.strip!="")
					{
						res ~= iq{@$(row[dir]): }.text; 
						foreach(a; row[2..$].enumerate.filter!q{a.value.strip!=""})
						{
							string decl = a.value; 
							if(stage=="geom" && dirStr=="in") decl ~= q{[]}; 
							res ~= iq{layout(location=$(a.index)) $(dirStr) $(decl); }.text; 
						}
					}
				}
			}
		...
		{
			mixin SmartParent!q{const string layerName=""};  //note, this is a Root parent.
		...
		{
			mixin SmartParent!q{@PARENT VulkanPhysicalDevice physicalDevice}; 
		...
		{
			mixin SmartChild!q{ @PARENT VulkanPhysicalDevice physicalDevice }; 
		...
		class VulkanDevice
		{
			...
			void _construct()
			{
				...
				static makeDeviceQueueCreateInfo(R)(R queueItems)
				{
					...
					return (
						mixin(體!((VkDeviceQueueCreateInfo),q{
	...
	version(/+$DIDE_REGION+/all)
	{
		...
		
		class VulkanGraphicsShaderModules
		{
			mixin SmartChild!q{
				@PARENT VulkanDevice 	device, 
				const(ubyte)[] 	binary
			...
			protected
			{
				union 
				{
					mixin(iq{VulkanShaderModule $(stageField.join(',')); }.text); 
					VulkanShaderModule[stageField.length] allShaderModules; 
				} 
				
				void clearShaderModules()
				{ foreach(ref m; allShaderModules[]) m.free; } 
				...
				void reload()
				{
					...
					foreach(i, ref m; allShaderModules)
					{
						...
						{
							enforce(
								(*data).startsWith(x"03 02 23 07"), 
								"Invalid SPIR-V binary format."
							); 
							m = new VulkanShaderModule(device, *data); 
						}
					}
				} 
			} 
			
			auto shaderStageFlagBits()
			=> allShaderModules[].enumerate.filter!q{a.value}
				.map!((a)=>(stageFlag[a.index])).fold!q{a|b}(VkShaderStageFlags.init); 
			
			auto pipelineShaderStageCreateInfos()
			=> allShaderModules[].enumerate.filter!q{a.value}
				.map!((a)=>(
				mixin(體!((VkPipelineShaderStageCreateInfo),q{
					stage 	: stageFlag[a.index],
					_module 	: a.value,
					pName 	: "main",
				}))
			)).array; 
