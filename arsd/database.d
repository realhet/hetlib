/+
	+
		Generic interface for RDBMS access. Use with one of the implementations in [arsd.mysql], 
		[arsd.sqlite], [arsd.postgres], or [arsd.mssql]. I'm sorry the docs are not good, but a little bit goes a long way:
		...
		with the exception of binary data,
		...
		History:
			...
			On June 7, 2023 (dub 11.0), I started the process of moving away from strings as the inner storage unit. 
			This is a potentially breaking change, but you can use `.toString` to convert as needed and `alias this` will 
			try to do this automatically in many situations. See [DatabaseDatum] for details. This transition is not yet complete.
...
import std.string, std.variant; 
import core.interpolation: InterpolationHeader, InterpolationFooter, InterpolatedLiteral, InterpolatedExpression; 

import het.arsd.core; 
...
interface Database
{
	...
	final ResultSet query(Args...)(InterpolationHeader header, Args args, InterpolationFooter footer)
	...
	/+
		+ Escapes binary data for inclusion into a sql string. Note that unlike `escape`, the 
		returned string here SHOULD include the quotes.
	+/
	string escapeBinaryString(const(ubyte)[] sqlData); 
	/+
		+ turns a systime into a value understandable by the target database as a timestamp 
		to be concated into a query. so it should be quoted and escaped etc as necessary
	+/
...
/+
	+
		...
		History:
			Added June 2, 2023 (dub v11.0). Prior to this, it would always use `string`. 
			This has `alias toString this` to try to maintain compatibility.
+/
struct DatabaseDatum
{
	...
	/+
		+ These are normally constructed by the library, so you shouldn't 
		need these constructors. 
		If you're writing a new database implementation though, here it is.
	+/
	...
	/+
		+
				For compatibility with earlier versions of the api, all data can easily 
				convert to string implicitly and opCast keeps to!x(this) working.
...
struct Row
{
	...
	/+
		+
				...
				won't want the format this prints in (and don't rely upon it, as it is subject to change 
				at any time without notice!), but it might be useful for use with `writeln`.
	+/
	string toString()
	...
	/+
		+
				...
				History:
					Prior to June 11, 2023 (dub v11.0), the order of iteration was undefined. It is now 
					guaranteed to be in the same order as it was returned by the database (which is 
					determined by your original query). Additionally, prior to this date, the datum was 
					typed `string`. `DatabaseDatum` should implicitly convert to string, so your code is 
					unlikely to break, but if you did specify the type explicitly you may need to 
					update your code.
		
					The overload with one argument, having just the datum without the name, was 
					also added on June 11, 2023 (dub v11.0).
	+/
	...
	/+
		+
				...
				I'd recommend against using these in new code. I wrote them back around 2011 as a 
				hack for something I was doing back then. Among the downsides of these is type 
				information loss in both functions (since strings discard the information tag) and 
				column order loss in `toAA` (since D associative arrays do not maintain any defined 
				order). Additionally, they to make an additional copy of the result row, which you may 
				be able to avoid by looping over it directly.
...
interface ResultSet
{
	...
	/* deprecated */ final ResultSet byAssoc()
	{ return this; } 
} 
...
abstract class ConnectionPoolBase : het.arsd.core.SynchronizableObject
{
...
/+
	+
		PooledConnection is an RAII holder for a database connection that is automatically 
		recycled to the pool it came from (unless you [discard] it).
...
struct PooledConnection(ConnectionPoolType)
{
	...
	/+
		+
				Indicates you want the connection discarded instead of returned 
				to the pool when you're finished with it.
	...
	~this()
	{
		...
		{
			/+
				 FIXME: a connection must not be returned to the pool unless it is both alive 
				and idle; any pending query work would screw up the next user
			+/
			/+
				 it is the user's responsibility to live with other state though like prepared 
				statements or whatever saved per-connection.
			+/
	...
	template query(string file = __FILE__, size_t line = __LINE__, Args...)
	{
		...
		auto query(InterpolationHeader ihead, Args args, InterpolationFooter ifoot)
...
private Variant[] variantsFromInterpolatedArgs(Args...)(Args args)
{
	...
	foreach(arg; args)
	{
		...
		{}
		else static if(is(typeof(arg) == InterpolationFooter)) {}
		else static if(is(typeof(arg) == InterpolatedLiteral!sql, string sql)) {}
		else static if(is(typeof(arg) == InterpolatedExpression!code, string code)) {}
		else static if(is(typeof(arg) == AdHocBuiltStruct!(tag, names, Values), string tag, string[] names, Values...)) {
...
private string sqlFromInterpolatedArgs(Args...)()
{
	...
	foreach(arg; Args)
	{
		...
		else static if(is(arg == InterpolatedLiteral!sql, string sql)) ret ~= sql; 
		else static if(is(arg == InterpolatedExpression!code, string code)) {}
		else static if(is(arg == AdHocBuiltStruct!(tag, names, values), string tag, string[] names, values...)) {
			static if(tag == "VALUES")
			{
				ret ~= "("; 
...
{
	override string sql() const
	{ return q; } 
	override string file() const
	{ return file_; } 
	override size_t line() const
	{ return line_; } 
} 
...
/+
	+
		Parent class of various forms of errors you can get when using the database.d 
		library. It may be thrown generically when other details are not provided by a driver.
...
/+
	+
		Thrown when your sql query has reached the database server, but failed to run 
		there for some reason.
	
		It is possible for this to be thrown on a connection problem in a query too, if 
		the driver didn't differentiate the cause.
...
/+
	+
		Thrown when you use result data incorrectly. These almost always are preventable, 
		but may be the result of a schema change and a `select *` query too.
...
class ConnectionPool(alias connectionFactory) : ConnectionPoolBase
{
	...
	bool checkQueries()(DriverType db) shared
	{
		bool succeeded = true; 
		/+
			/+Todo: het - reimplement this!+/
			import arsd.postgres; /+
				 FIXME is this really postgres only? looks like sqlite has no similar function... 
				maybe make a view then sqlite3_table_column_metadata ?
			+/
...
string toSql(Database db, Variant a)
{
	...
	string binary(const(ubyte)[] b)
	{
		if(b is null)
		return "NULL"; 
		else return db.escapeBinaryString(b); 
	} 
	
	auto v = a.peek!(void*); 
	if(v && (*v is null))
	{ return "NULL"; }else if(auto t = a.peek!(SysTime))
	{ return db.sysTimeToValue(*t); }else if(auto t = a.peek!(DateTime))
	{
		// FIXME: this might be broken cuz of timezones!
		return db.sysTimeToValue(cast(SysTime) *t); 
	}else if(auto t = a.peek!(ubyte[]))
	{ return binary(*t); }else if(auto t = a.peek!(immutable(ubyte)[]))
	{ return binary(*t); }else if(auto t = a.peek!string)
	{
		auto str = *t; 
		if(str is null)
		return "NULL"; 
...
int updateOrInsert(Database db, string table, string[string] values, string where, UpdateOrInsertMode mode = UpdateOrInsertMode.CheckForMe, string key = "id")
{
	...
	}else
	{
		...
		foreach(column, value; values)
		{
			...
			else updateSql ~= identifierQuote ~ db.escape(column) ~ identifierQuote ~ " = '" ~ db.escape(value) ~ "'"; 
		}
...
string fixupSqlForDataObjectUse(string sql, string[string] keyMapping = null)
{
	...
	if(idx != -1)
	{
		...
		start = i; 
		while(
			i < sql.length && !(sql[i] > 'A' && sql[i] <= 'Z')// if not uppercase, except for A (for AS) to avoid SQL keywords (hack)
		)
		i++; 
...
class DataObject
{
		...
		if((field.length < 8 || field[0..8] != "id_from_") && field != "popFront")
	{
		static if(__traits(compiles, t is null))
		{
			if(t is null)
			setImpl(field, null); 
			else setImpl(field, to!string(t)); 
		}else
		...
		if(updateCount)
		{
			...
			foreach(tbl, values; toUpdate)
			{
				...
				foreach(i, key; keys)
				{
					...
					if(f is null)
					where ~= keyField ~ " = NULL"; 
					else where ~= keyField ~ " = '"~db.escape(f)~"'"; 
	...
	DataObject addNew()
	{
		...
		n.fields[k] = v; 
		else n.fields[k] = null; // don't copy ids
...
string DataObjectField(T, string table, string column, string aliasAs = null)()
{
	...
	if(aliasAs is null)
	aliasAs_ = column; 
	else aliasAs_ = aliasAs; 
	return `
		@property void `~aliasAs_~`(`~T.stringof~` setTo) {
			super.set("`~table~`", "`
	~column~`", to!string(setTo));
		}

		@property `~T.stringof~` `~aliasAs_~` () {
			return to!(`
	~T.stringof~`)(super.select("`~table~`", "`~column~`"));
		}
	`; 
} 
...
string getCreateTable(string sql, string tableName)
{
	...
	Field* current = new Field(); /+
		 well, this is interesting... under new DMD, not using new breaks it 
				in CTFE because it overwrites the one entry!
	+/
	while(word != ")" || parens)
	{
		if(word == ")")
...
{
	alias SimpleDataObject!(tableName, StructFromCreateTable!(sql, tableName)) 
	...
	foreach(memberName; __traits(allMembers, T))
	{
		...
		foreach(attr; __traits(getAttributes, member))
		{
			static if(is(attr == DbSave))
			{
				...
				else static if(is(typeof(member) == bool)) { __traits(getMember, t, memberName) = row[memberName][0] == 't'; }else static if(is(typeof(member) == Html))
				{ __traits(getMember, t, memberName).source = row[memberName]; }else static if(is(typeof(member) == DateTime))
	...
	foreach(memberName; __traits(allMembers, T))
	{
		...
		foreach(attr; __traits(getAttributes, member))
		{
			static if(is(attr == DbSave))
			{
				...
				else static if(is(typeof(member) == Html)) { obj.opDispatch!memberName(__traits(getMember, t, memberName).source); }else static if(is(typeof(member) == DateTime))
...
void fillData(T)(void delegate(string, string) setter, T obj, string name)
{
	...
	{ fillData(setter, obj.toISOExtString(), name); }else static if(is(T == Html))
	{ fillData(setter, obj.source, name); }else static if(is(T == struct))
	{
