...
+/
module het.arsd.mssql; 

version(Windows)
pragma(lib, "odbc32"); 
else	pragma(lib, "odbc"); 
...
/+import etc.c.odbc.sql, etc.c.odbc.sqlext; These are Deprecated+/
...
class MsSql : Database
{
	...
	this(string connectionString)
	{
		...
		auto ret = SQLDriverConnect(
			conn, null, cast(ubyte*)connectionString.ptr, cast(short)SQL_NTS,
			null, 0, null,
			SQL_DRIVER_NOPROMPT 
		); 
...
/+
	void omg()
	{
		...
		retcode = SQLDriverConnect(
			hdbc, null, cast(ubyte*)"DSN=PostgreSQL30Postgres".ptr, cast(short)SQL_NTS,
			null, 0, null,
			SQL_DRIVER_NOPROMPT 
		); 
