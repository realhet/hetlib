...
version(/+$DIDE_REGION+/all) {
	...
	import std.variant, std.datetime; 
	...
	class MySql : Database
	{
			this(string host, string user, string pass, string db, uint port = 0)
		{
			...
			enforce!(DatabaseConnectionException)(
				mysql_real_connect(
					mysql, toCstring(host), toCstring(user), 
					toCstring(pass), toCstring(db), port, null, 0
				),
				error()
			); 
...
version(/+$DIDE_REGION+/all) {
	...
	enum_field_types getMySqlType(T)()
	{
		...
		return enum_field_types.MYSQL_TYPE_TINY; 
		else static if(is(T == short) || is(T == ushort))	return enum_field_types.MYSQL_TYPE_SHORT; 
		else static if(is(T == int) || is(T == uint))	return enum_field_types.MYSQL_TYPE_LONG; 
		else static if(is(T == long) || is(T == ulong))	return enum_field_types.MYSQL_TYPE_LONGLONG; 
		else static if(is(T == string))	return enum_field_types.MYSQL_TYPE_STRING; 
		else static if(is(T == float))	return enum_field_types.MYSQL_TYPE_FLOAT; 
		else static if(is(T == double))	return enum_field_types.MYSQL_TYPE_DOUBLE; 
		else
	...
	///
	Ret queryOneRow(Ret = Row, DB, string file = __FILE__, size_t line = __LINE__, T...)(DB db, string sql, T t) 
		if(
		(is(DB : Database))
	...
	)
	{
		...
		static assert(0, "Unsupported single row query return value, " ~ Ret.stringof); 
	} 
