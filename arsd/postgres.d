...
+/
module het.arsd.postgres; 

version(Windows)
pragma(lib, "libpq"); 
else pragma(lib, "pq"); 

public import het.arsd.database; 

import std.string, std.exception, std.datetime, std.variant; 
...
{
	/+
		+ `dbname=your_database_name` is probably the most common connection string. See section 
			"33.1.1.1. Keyword/Value Connection Strings" on https://www.postgresql.org/docs/10/libpq-connect.html
	+/
	...
	*/
	...
	string escapeBinaryString(const(ubyte)[] data)
	{
		...
		if(len == 0)
		res = "''"; 
		else res = cast(string) ("'" ~ buf[0 .. len - 1] ~ "'"); // gotta cut the zero terminator off
...
/+
	# when it changes from lowercase to upper case, call that a new word. 
		or when it goes to/from anything else and underscore or dashes.
+/
...
import het.arsd.core : LimitedVariant, PackedDateTime, SimplifiedUtcTimestamp; 
	...
	switch(platformSpecificTag)
	{
		...
		case DATEOID: 
			...
			return LimitedVariant(d); /+
			 might want a different type so contains shows 
			the thing without checking hasDate and hasTime
		+/
...
string copyCString(const char* c, int actualLength = -1) @system
{
	...
	else { ret = a[0..actualLength].idup; }
...
extern(C)
{
	...
	PGresult* PQexecPrepared(
		PGconn*, const char* stmtName, int nParams, const char** paramValues, 
	...
	int PQsendQueryParams(
		PGconn* conn, const char* command, int params, const Oid* paramTypes, 
