/+
	+
		...
		Compile with `-version=sqlite_extended_metadata_available` if your sqlite is compiled with the 
		`SQLITE_ENABLE_COLUMN_METADATA` C-preprocessor symbol.
	
		If you enable that, you get the ability to use the queryDataObject() function with sqlite. (You can still 
		use DataObjects, but you'll have to set up the mappings manually without the extended metadata.)
...
import std.datetime, std.variant; 
...
class Sqlite : Database
{
	  public: 
		...
		+
				...
				History:
					The `flags` argument was ignored until July 29, 2022. (This function was originally written over 11 years ago, 
					when sqlite3_open_v2 was not commonly supported on some distributions yet, and I didn't notice to revisit it for ages!)
	+/
		...
		override string escape(string sql) @system
	{
		if(sql is null)
		return null; 
		char* got = sqlite3_mprintf("%q", toStringz(sql)); /+
			 FIXME: might have to be %Q, need to check this, 
			but I think the other impls do the same as %q
		+/
		auto orig = got; 
		string esc; 
		while(*got)
	...
	Row front()
	{
		...
		foreach(c; rows[position])
		{
			...
			r.row ~= DatabaseDatum(cast(string) *t); 
				...
				auto d = c.peek!double
				/+
					 17 significant decimal digits are enough 
					to not lose precision (IEEE 754 section 5.12.2)
				+/
			)	r.row ~= DatabaseDatum(format!"%.17s"(*d)); 
			else { r.row ~= DatabaseDatum(c.coerce!(string)); }
		}
...
struct Statement
{
	...
	*/
	...
	{ bind(bindNameLookUp(name), value); } 
			void bind (const char[] name, const byte[] value)
	{ bind(bindNameLookUp(name), value); } 
			void bind (const char[] name, const ubyte[] value)
	{ bind(bindNameLookUp(name), value); } 
		...
		void bind(int col, Variant v)
	{
		if(v.peek!long)
		bind(col, v.get!long); 
		else if(v.peek!ulong)	bind(col, v.get!ulong); 
		else if(v.peek!int)	bind(col, v.get!int); 
		else if(v.peek!(const(int)))	bind(col, v.get!(const(int))); 
		else if(v.peek!bool)	bind(col, v.get!bool ? 1 : 0); 
		else if(v.peek!DateTime)	bind(col, v.get!DateTime.toISOExtString()); 
		else if(v.peek!string)	bind(col, v.get!string); 
		else if(v.peek!float)	bind(col, v.get!float); 
		else if(v.peek!double)	bind(col, v.get!double); 
		else if(v.peek!(byte[]))	bind(col, v.get!(byte[])); 
		else if(v.peek!(ubyte[]))	bind(col, v.get!(ubyte[])); 
		else if(v.peek!(immutable(ubyte)[]))	bind(col, v.get!(immutable(ubyte)[])); 
		else if(v.peek!(void*) && v.get!(void*) is null)	bind(col, null); 
		else	bind(col, v.coerce!string); 
