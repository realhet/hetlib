...
string _DATALOGMIXIN(string p) { return `_DATALOGMIXINFUNCT(`~p~`, q{`~p~`})`; }

string _DATABRKMIXIN(string p) { return `_DATALOGMIXINFUNCT(`~p~`, q{`~p~`})`; }
...
class DebugLogClient {
	...
	public:
		...
		*cast(uint*)(packet.ptr) = cast(uint)s.length;
		...
		void setExeHwnd(void* hwnd) { if(data) data.exe_hwnd = cast(int)hwnd; }
}
...
class DebugLogServer {
	...
	ubyte[] CircBuf_getLog(ref uint tail, ref uint head, in uint cap, ubyte* buf) {
		...
		uint capacity() { return cap; }
		uint length() { return head-tail; }
		uint canGet() { return length; }
		
		uint truncate(uint x) { return x % cap; }
		
		void Move(in void *source, void *destination, uint num) { (cast(ubyte*)destination)[0 .. num][]=(cast(const(ubyte)*)source)[0 .. num]; }
	...
	string pingLedStateText() { return pingLedState[].enumerate.map!(a => a.value ? a.index.text : "_").join; }
	...
	int getLogCount() { return logEvents.length.to!int; }
	string getLogStr(int idx) { return logEvents.get(idx); }
	bool logChanged() { return logChanged_; } //signal to redraw log list
	
	void setPotiValue(int idx, float val) { if(data && idx.inRange(data.poti)) data.poti[idx] = val; }
	
	void clearExit() { data.forceExit = 0; }
	void forceExit() { data.forceExit = 1; }
	...
	void forcedStop() {
		...
		with(data) {
			dide_ack	      = -1;
			forceExit	      =	1;
			exe_waiting		= 0; //220429: This is needed when the exe is dead. It resets the IDE *break* state.
		}
	}
	
	bool isExeWaiting() { return data && data.exe_waiting!=0; }
