...
//Standard arrows
private int encodeArrowStyle(int headArrow, int tailArrow, int centerNormal) { return headArrow<<0 | tailArrow<<2 | centerNormal<<4; }
...
class Drawing {
	  //Drawing /////////////////////////////////////////////////////////
		...
		Drawing clone() { return new Drawing(this); }
	...
	////////////////////////////////////////////////////////////////////////////////
		...
		//auto	arrowSize = vec2(8, 2.5);   //this is fixed in the shader
				...
				float fontWeight = 1.0;
														//1            2        4            8            16                                                            32
				static foreach(i, s; ["MonoSpace", "Italic", "Underline", "StrikeOut", "Image", /+from here: customshader if image, or for font-> +/ "Transparent"]) { mixin("@property bool font*() const { return (fontFlags>>#)&1; }  @property void font*(bool b){ fontFlags = cast(ubyte) (fontFlags & ~(1<<#) | (cast(int)b << #)); }".replace('*', s).replace('#', i.text)); }
			...
			return *(cast(long*) &this);
				...
				/// ditto
				void quickRestore(long data) { *(cast(long*) &this) = data; }
	}
	...
	////////////////////////////////////////////////////////////////////////////////
		...
		private auto exportDrawingObjs() { return buffers.allObjs; }
	...
	*/
		...
		static if(is(T==RGB8)||is(T==RGBA8)) { mixin(genRgbGraph!"vGraph"); }else static if(is(T==vec2) || is(T==ivec2)) { foreach(i, const v; data) lineTo(x0+v.y*xScale, y0+v.x*yScale, !i); }else {
		...
		if(data.empty) return;
		static if(is(T==RGB8)||is(T==RGBA8)) { mixin(genRgbGraph!"hGraph"); }else static if(is(T==vec2) || is(T==ivec2)) { foreach(i, const v; data) lineTo(x0+v.x*xScale, y0+v.y*yScale, !i); }else {
	...
	+/
		...
		void fillConvexPoly(in vec2[] p) { foreach(i; 2..p.length) fillTriangle(p[0], p[i], p[i-1]); }
	...
	//Text ////////////////////////////////////////////////////////////////////////
		...
		void textOut(float x, float y, string text, float width = 0, HAlign align_ = HAlign.left, bool vertFlip = false) { textOut(vec2(x, y), text, width, align_, vertFlip); }
		void textOut(in ivec2 p, string text, float width = 0, HAlign align_ = HAlign.left, bool vertFlip = false) { textOut(vec2(p), text, width, align_, vertFlip); }
		...
		void textOut(float x, float y, string[] lines, float width = 0, HAlign align_ = HAlign.left, bool vertFlip = false) { textOut(vec2(x, y), lines, width, align_, vertFlip); }
	
		void textOutMulti(float x, float y, string lines, float width = 0, HAlign align_ = HAlign.left, bool vertFlip = false) { textOut(x, y, lines.splitLines, width, align_, vertFlip); }
	
		void textOutMulti(vec2 p, string lines, float width = 0, HAlign align_ = HAlign.left, bool vertFlip = false) { textOut(p, lines.splitLines, width, align_, vertFlip); }
	...
	//Draw Bitmap ////////////////////////////////////
		...
		void drawClearType(Bitmap bmp, float x0=0, float y0=0) { drawPixels(bmp, x0, y0, true); }
	...
	////////////////////////////////////////////////////////////////////////////////
		...
		q{
			...
			*/ //NV error: error C7549: OpenGL does not allow C style initializers
			...
			void emitTriangle(vec2 a, vec2 b, vec2 c) { emit(a); emit(b); emit(c); }
			...
			if(arrowStyle!=0.0 || true) {
				...
				if(arrowTail==1.0) EmitTrapezoid(p0+fwd, p0, -side,	-hside);else if(arrowTail==2.0)
				EmitTrapezoid(p0+side, p0-side,	hdir,  hdir);
				
				if(arrowHead==1.0) EmitTrapezoid(p1-fwd, p1,  side,  hside);else if(arrowHead==2.0)
				EmitTrapezoid(p1+side, p1-side,  hdir,  hdir);
				
				if(centerNormal==1.0) EmitTrapezoid(pc     , pc-side,	 hdir,	 hdir);else if(centerNormal==2.0)
				EmitTrapezoid(pc+side, pc     ,	 hdir,	 hdir);else if(centerNormal==3.0)
				EmitTrapezoid(pc+side, pc-side,	 hdir,	 hdir);
			}
			
		}
		
			vec2 evalBezier2(vec2 A, vec2 B, vec2 C, float t) { return mix(mix(A, B, t), mix(B, C, t), t); }
			...
			*/
			...
			if(samplingLevel==-1) {
				...
				if(stConfig==8) finalColor = vec4(texel.rgb, fontColor.a);
					...
					finalColor = isTransparent ? vec4(fontColor.rgb, texel.a) : mix(bkColor, fontColor, texel.a);
			...
			if(fColor2.w==0) {
				...
				if(isFont) { isTransparent = (fontFlags&32)!=0; }else {
		...
		@property void customShader(string s) { if(chkSet(customShader_, s)) { customShaderChanged = true; } }
		...
		Shader getShader() {
			...
			if(!sh || customShaderChanged) {
				...
				try { sh = recompile(No.ignoreCustomShader); }catch(Exception e) {
					if(!shDefault)
					try { shDefault = sh = recompile(Yes.ignoreCustomShader); }catch(Exception e2) {
						ERR(e2.simpleMsg);
...
deprecated class Logger {
	...
	void dump(string s) { log(s, LogType.Dump); }
	
	void code(string s) { log(s, LogType.Code); }
