...
@property void font*(bool b){ fontFlags = cast(ubyte) (fontFlags & ~(1<<#) | (cast(int)b << #)); }"
	...
	{ lineTo(p);	 }
	void lineTo(in ivec2 p, bool isMove)
	{ lineTo(p, isMove); }
	
	void moveTo(in vec2 p)
	{ lineCursor = p; }
	void moveTo(in ivec2 p)
	{ moveTo(p); }
	...
	{ lineTo(vec2(x, y)); }
	void lineTo(float x, float y, bool isMove)
	{ lineTo(vec2(x, y), isMove); }
	void moveTo(float x, float y)
	{ moveTo(vec2(x, y)); }
	void lineRel(float x, float y)
	{ lineRel(vec2(x, y)); }
	...
	*/
	...
	{ drawRect(b.low, b.high); }
	void drawRect(in ibounds2 b)
	{ drawRect(bounds2(b)); }
	
	void drawX(float x0, float y0, float x1, float y1)
	{ line(x0, y0, x1, y1); line(x0, y1, x1, y0); }
	void drawX(in vec2 a, in vec2 b)
	{ drawX(a.x, a.y, b.x, b.y); }
	void drawX(in bounds2 b)
	{ drawX(b.low, b.high); }
	void drawX(in ibounds2 b)
	{ drawX(bounds2(b)); }
	...
	{ fillRect(a.x, a.y, b.x, b.y); }
	void fillRect(in bounds2 b)
	{ fillRect(b.low, b.high); }
	void fillRect(in ibounds2 b)
	{ fillRect(bounds2(b)); } //todo: ibounds2 automatikusan atalakulhasson bounds2-re
