...
vec2 restrictPos_normal(T1, T2)(in Vector!(T1, 2) p, in Bounds!(Vector!(T2, 2)) bnd) { return p.clamp(bnd.low, bnd.high); }
...
int distManh(in ibounds2 b, in ivec2 p) { with(b) return max(max(left-p.x, p.x-right, 0), max(top-p.y, p.y-bottom, 0)); }
