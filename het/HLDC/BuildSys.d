	...
	private bool _processLine(string line){
		try{
			if(line.isWild(`?:\?*.d*(?*,?*):?*`)){
				...
				if(auto orig = location in messages){ //duplicated location: append the contents to the end of the existing message
					orig.message ~= "\n" ~ content.strip;
				}else	if(content.startsWith("  ")){ //supplemental message
