...
auto predecodeLdcOutput(string input) {
	...
	foreach(i; 0..lines.length.to!int) {
		...
		) { fetchLastMessage(i); }else stack ~= lines[i];
	}
	fetchLastMessage;
...
void test_predecodeLdcOutput() {
		...
		deprecated("cause") void depr1() { print("a"); }
...
class Executor {
	...
	void kill() {
		if(!isFinished) {
			if(pid)
			try { std.process.kill(pid); }catch(Exception e) {
...
int spawnProcessMulti2(File[] mainFiles, in string[][] cmdLines, in string[string] env, Path workPath, Path logPath, out string[] sOutput, bool delegate(int idx, int result, string output) onProgress/*returns enable flag*/, bool delegate(int inFlight, int justStartedIdx) onIdle/*return cancel flag*/) {
	...
	while(executors.update(onProgress)) {
		...
		if(!cancelled)
		if(runningCnt==0 || ((now-lastLaunchTime).value(second)>0.05f && runningCnt<GetNumberOfCores-1 && GetCPULoadPercent<90 && GetMemAvailMB>512)) {
...
struct BuildSystem {
		...
		void log(T...)(T args)
	{
		...
		mf.remove;
		if(settings.generateMap) { mapFile = mf; }
		...
		//resFile
		if(resFiles.length>0) resFile = targetFile.otherExt(".res");
		else
		resFile = File("");
		...
		if(!isExe && !isDll) { enforce(isTarget, "Main project file must start with target declaration (//@EXE or //@DLL) with an optional projectName."); }else { enforce(!isTarget, "Target declaration (//@EXE or //@DLL) is already specified."); }
		...
		void compile(File[] srcFiles, File[] cachedFiles) //Compile ////////////////////////
	{
		...
		//////////////////////////////////////////////////////////////////////////////////////
			...
			srcFiles, cmdLines, null, /*working dir=*/Path.init, /*log path=*/workPath, sOutputs, (idx, result, output){
	...
	{
		{
			...
			if(targetFile.exists) { if(settings.killExe && !disableKillProgram) { enforce(killDeleteExe(targetFile), "Failed to close target process."); }else { enforce(false, "Unable to delete target file."); } }
...
void buildSystemWorker() {
	...
	while(!isDone) {
		receive(
			(MsgBuildCommand cmd){ if(cmd==MsgBuildCommand.shutDown) isDone = true; },
			(immutable MsgBuildRequest req){
				...
				}catch(Exception e) { error = e.simpleMsg; }
...
struct CodeLocation {
	...
	string toString() const { return file.fullName ~ (column ? format!"(%d,%d)"(line, column) : line ? format!"(%d)"(line) : ""); }
}
...
class BuildResult {
	...
	string repairFileCase(string fn) {
		return correctFileCase.require(fn, (){ if(auto idx = allFiles.map!"a.fullName".countUntil!"icmp(a,b)==0"(fn)+1) { return allFiles[idx-1].fullName; }else { return File(fn).actualFile.fullName; } }());
		...
		try {
			if(line.isWild(`?:\?*.d*(?*,?*):?*`)) {
				...
				if(auto orig = location in messages) {
					...
					if(type == BuildMessageType.warning && msg.isWild("C preprocessor directive `#*` is not supported")) return false;
	...
	string dump() {
		...
		static if(0) {
			 auto f = mainFile;
			if(f in remainings && remainings[f].length) { remainings[f].each!(a => res ~= a~"\n"); }
		}else {
