...
struct ClickDetector {
	...
	void update(bool state, DateTime t=DateTime.init) {
		...
		}else if(released) { if(longPressing.chkClear) { longClicked = clicked = true; }else { if(clickCount==0) clicked = true; } }
...
int keyModifierCode(string key) {
	...
	foreach(i, a; keyModifiers) { if(key==a || key[0].among('L', 'R') && key[1..$]==a) { return 1<<cast(int)i; } }
	return 0;
}

int keyModifierMask(in string[] keys) { int res; foreach(k; keys) res |= k.keyModifierCode; return res; }
...
struct KeyComboEntry {
	...
	bool pressed ()	 { return valid &&	inputs.keyModifierMask==keyModifierMask && entries(keys.length-1).pressed; }
...
class InputEntry {
	...
	void _updateRepeated(Time now) { repeated_ = inputs.repeatLogic(active, pressed, repeatNextTime); }
...
class InputManager {
	...
	public:
		...
		void clearDeltas() { foreach(e; entries) e.lastValue = e.value; }
	
		auto opIndex(string name) {
		 //todo: this should be const. And there should be access(name) which has read/write access.
		
		struct InpitEntryWrapper {
			 //note: this wrapper is needed for opCast!(bool) to work properly. For classes, D doesn't call opCast, it checks the pointer.
			alias entry this;
			InputEntry entry;
	...
	public: //standard stuff
		...
		bool repeatLogic(in bool active, in bool pressed, ref Time repeatNextTime) { return repeatLogic(active, pressed, repeatNextTime, repeatDelay1, repeatDelay2); }
...
class DInputWrapper {
	private:
		const bool loaded;
		extern(Windows) { HRESULT function(HINSTANCE hinst, uint dwVersion, REFIID riidltf, out IDirectInput8A ppvOut, IUnknown punkOuter) DirectInput8Create; }
	
		IDirectInput8A directInput8;
		IDirectInputDeviceA directMouse, directJoystick;
	
		static string nameOf(in GUID g) {
		static foreach(s; "XAxis,YAxis,ZAxis,RxAxis,RyAxis,RzAxis,Slider,Button,Key,Pov".split(',')) { mixin("if(g==IID_%s) return `%s`;".format(s, s)); }
		import std.uuid;
...
void mouseMoveRel(float dx, float dy) {
	...
	if(slowMouseEnabled) { slowMousePos += vec2(dx, dy); }else {
...
private __gshared {
	...
	void slowMouseUpdate() {
		...
		}else { slowMousePos = winMousePos + vec2(.5, .5); }
	}
...
class PotiInputHandler: InputHandlerBase {
	...
	public:
		...
		override void update() { foreach(i; 0..ie.length) ie[i].value = dbg.getPotiValue(i); }
}
		...
		switch(type) {
			...
			if(task) {
				...
				res = true;
								...
								break;
			case 'm': if(modifier) { res = chkSet(*modifier, inputs.active(key)); }
								break;
...
struct ActionManager {
		...
		void importKeyMap(string[string] map)
	{
		...
		if(enabled && actGroup.enabled && ac.enabled) { if(ac.execute) { m_changed |= true; } }
	}
	...
	public:
		...
		foreach(ref grp; groups) {
			s ~= "  "~grp.name~"\n";
			foreach(ref ac; grp.actions) { s ~= format("    %-28s: %s\n", ac.name, ac.key); }
		}
		return s;
	}
...
@("kbd/mouse access") public	 {
	...
	extern(Windows) LRESULT LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam) nothrow
	{
		...
		try {
			...
			}else { if(log) writeln("wparam", wParam); }
