		...
		void drawPin(vec2 p, string label) {
			  string colorName = pinColorMap.get(label, "");
				...
				colorName = wild[1];
				 ...
				 RGB c0, c1;
				 if(colorName=="") { c0 = c1 = clBody; }else if(colorName.isWild("*-*")) {
				c0 = wild[0].toWireColor;
				c1 = wild[1].toWireColor;
			}else { c0 = c1 = colorName.toWireColor; }
...
class ArduinoNanoProject {
				...
				YAlign.top, {
					...
					Row(
						YAlign.top, {
							...
							if(identifier!="") Row({ Btn({ Text(pin~" ");  Led(state != '0', state.among('0', '1') ? isInput ? clLime : clYellow : clGray); }, genericId(pin)); });
						}
					);
				}
			);
	...
	static auto getNanoPinReg(string pin) {
		...
		}catch(Exception e) { raise(format!"Invalid arduino pin: %s (%s)"(pin.quoted, e.simpleMsg)); }
