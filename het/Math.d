...
{//Vector ////////////////////////////////////
	...
	int CommonVectorLength(T...)()
	{
		...
		else	static if(!anyVector!T	) return 1; // assume all are scalar
		else{
...
{// Vector relational functions //////////////////////////////////////////
	...
	{
		static if(isVector!A)
			return a.generateVector!(bool, a => !a); //returns bvec, not vec, like in opengl
		else
			return !a;
	}
...
}version(/+$DIDE_REGION+/all)struct Image(E, int N) // Image struct //////////////////////////////////
{	
	version(/+$DIDE_REGION+/all)
	{//copied from dlang opSlice() documentation
		...
		@property {
			static if(N==1)
				...
				{ return size; } else int opDollar(size_t dim : 0)()
				{ return size[0];	}
			static if(N>=2) int opDollar(size_t dim : 1)()
			{ return size[1]; }
			static if(N>=3) int opDollar(size_t dim : 2)()
			{ return size[2]; }
		}
	}version(/+$DIDE_REGION+/all){
		// Index/Slice assign
...
{	
	private void unittest_Image() // image2D tests /////////////////////////////////
	{
		...
		{// delegate with void result: operation on Images
			...
			assert(image2D!q{ a = a*b+c; return vec2(1,2); } (im3, im2, 5)
				==
...
}version(/+$DIDE_REGION Functions+/all)
{
	...
	}version(/+$DIDE_REGION+/all)
	{
		...
		private void unittest_GeometricFunctions()
		{
			...
			assert(length(v[0]).approxEqual(sqrt(1^^2 + 2^^2))
	...
	}version(/+$DIDE_REGION+/all)
	{
		private void unittest_CommonFunctions()
		{
			...
			assert(cmp("abc", "abc")==0 && cmp("abc", "abcd")<0 && cmp("bbc", "abc"w)>0
			...
			assert(cmp(vec2(1),1)==vec2(cmp(1,1)) && cmp(vec2(1),2)==vec2(cmp(1,2))
			...
			assert(8.iota.map!(i => (smoothstep(ivec2(0, 2), 9, i)*100).iround )
				.equal([ivec2(0, 0), ivec2(3, 0), ivec2(13, 0), ivec2(26, 6),
