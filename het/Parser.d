...
struct DPaths {
	...
	*/
		...
		void init() { importPaths.clear; }
	
		void includeDelimiters() { foreach(ref p; importPaths) p = includeTrailingPathDelimiter(p); }
		...
		void addImportPathList(string paths) { foreach(path; paths.split(';')) { addImportPath(path); } }
		...
		bool isStdFile(in File f) { return f.fullName.isWild(stdImportPath~"*"); }
}
...
struct BracketHierarchyProcessor {
	...
	{ if(!wasError && err!="") { errorStr = format("%s(%s:%s): %s", fileName, token.line, token.posInLine, err); } }
	
		void process(ref Token t)
	{
		if(isClosingBracket(t)) {
			if(queue.empty) { addError(&t, format(`Unpaired closing bracket "%s".`, t.source)); }else if(t.id!=queue.back.endOp) { addError(&t, format(`Unpaired closing bracket "%s". Expected "%s".`, t.source, opStr(queue.back.endOp))); }else {
		...
		void finalize() { if(queue.length) with(queue.back) { addError(startToken, format(`Closing bracket expected: "%s".`, opStr(endOp))); } }
...
struct ImportBind { string alias_, name; }
...
struct ImportDecl {
	...
	bool isStdModule () const { return nameStartsWith("std"); } //todo: make these automatic from list like "std, etc, ldc, code"
	bool isEtcModule () const { return nameStartsWith("etc"); }
	bool isCoreModule() const { return nameStartsWith("core"); }
	bool isLdcModule () const { return nameStartsWith("ldc"); }
	bool isUserModule() const { return !isStdModule && !isCoreModule && !isEtcModule && !isLdcModule; }
...
class Parser {
		...
		auto collectBuildMacrosAndTodos(out string[] macros, out string[] todos) //updates Token.isBuildCommant
	{
		...
		foreach(ref cmt; tokens) {
			if(cmt.isComment) {
				if(cmt.posInLine==0 && cmt.source.startsWith("//@")) {
					...
					if(!m.empty) { t = "Todo";  s = m[1]; }else {
						m = cmt.source.matchFirst(rxOpt);
						if(!m.empty) { t = "Opt";  s = m[1]; }else {
							m = cmt.source.matchFirst(rxBug);
							if(!m.empty) { t = "Bug";  s = m[1]; }
						}
					}
					
					s = s.strip;
					if(!s.empty) { todos ~= format(`%s(%d,%d): %s: %s`, fileName, cmt.line+1, cmt.posInLine+1, t, s); }
				}
			}
		}
	}
		...
		bool nextSym() { if(!eof) { seek(actIdx+1); return true; }else { return false; } }
		...
		string[] res;
		do { res ~= expectIdentifier; }while(acceptOp(opSeparator));
		return res;
	}
