...
struct JsonDecoderState {
	...
	string errorMsg(string msg, int tokenIdx=-1) {
		...
		}else if(moduleName!="") { location = quoteIfNeeded(moduleName) ~ ": "; }
		
		return (location ~ " " ~ msg).strip;
	}
	
	void raise(string msg, int tokenIdx=-1) { throw new Exception(errorMsg(msg, tokenIdx), srcFile, srcLine); }
...
void streamDecode_json(Type)(ref JsonDecoderState state, int idx, ref Type data) {
	...
	string peekClassName(int[string] elementMap) {
		...
		}else { if(check("class")) return res; }
	...
	void getSign() { if(isOp!'-') { isNegative = true; idx++; } }
	...
	try {
		...
		static if(isFloatingPoint!T   ) {
		...
		}else static if(is(T == enum)       ) { data = actToken.data.get!string.to!Type; }else static if(isIntegral!T        ) {
			getSign;
			...
			+/
		}else static if(isSomeString!T      ) { data = actToken.data.get!string.to!Type; }else static if(isSomeChar!T        ) {
			...
			static if(is(T == class)) {
				{
					...
					*/
					...
					if(data is null) { static if(__traits(compiles, new Type)) { data = new Type; }else { raise("fromJson: Unable to construct new instance of: "~Type.stringof); } }
				}
			}
			...
			while(1) {
				...
				if(cnt>=data.length) {
					static if(isDynamicArray!T) {
						...
						if(state.errorHandling==ErrorHandling.raise) { throw new Exception("Static array overflow."); }else {
			...
			}else { if(data.length != cnt) throw new Exception("Static array size mismatch: %s < %s".format(cnt, data.length)); }
...
void streamAppend_json(Type)(ref string st, /*!!!!!*/in Type data, bool dense=false, bool hex=false, string thisName="", string indent="") {
	...
	//switch all possible types
				static if(isFloatingPoint!T	  ) { st ~= data.text_precise; }else static if(is(T == enum)	  ) { st ~= quoted(data.text); }else static if(isIntegral!T	  ) { if(hex) st ~= format!"0x%X"(data);else st ~= data.text; }else static if(isSomeString!T	  ) { st ~= quoted(data); }else static if(isSomeChar!T	  ) { st ~= quoted([data]); }else static if(is(T == bool)		) { st ~= data ? "true" : "false"; }else static if(isVector!T		 ) { streamAppend_json(st, data.components, dense || true, hex, "", indent); }else static if(isMatrix!T		 ) { streamAppend_json(st, data.columns   , dense || true, hex, "", indent); }else static if(isAggregateType!T	  ) {
		 //Struct, Class
		//handle null for class
...
string[] getPropertyValues(string filter = "true")(Property[] props, string rootPath="") {
	...
	foreach(a; props) {
		...
		if(auto ps = cast(PropertySet)a) { res ~= getChangedPropertyValues(ps.properties, fullName); }else { if(mixin(filter)) { res ~= fullName ~ '=' ~ a.asText; } }
	}
	return res;
}
...
string[] getChangedPropertyValues(Property[] props, string rootPath="") { return getPropertyValues!"chkClear(a.uiChanged)"(props, rootPath); }
