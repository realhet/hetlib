...
class Archiver{
  ...
  this(){
    ...
    // do a quick selftest at the very beginning
    __gshared static bool selfTest;
    if(chkSet(selfTest)){
      RNG rng;
      //const t0=QPS;
  ...
  void initStream(uint a, uint c){ seedStream = SeedStream(a, c); }
  ...
  ubyte[] createRecord(string header, in void[] data, string headerCompression, string dataCompression){
  ...
  auto decodeRecord(in uint[] data_, string dataCompression){
    ...
    try{
      ...
      uint[] fetchExactly(uint n){
        auto res = tryFetch(n);
        enforce(res.length == n, "Not enough input data");
        return res;
      }
  ...
  auto peekRecord(ref uint[] data){
    struct Res{
      bool isLeadIn, isLeadOut;
      uint headerLength, dataLength, seekBack;
      bool valid(){ return isLeadIn || isLeadOut; }
    }
