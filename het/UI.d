...
struct im {
	 static:
		...
		enforce(n.among(0, 1));
		with(targetSurfaces[n]) { actView = view; }
	}
		...
		static if(doTiming) { double tBeginFrame, tEndFrame, tDraw; }
		...
		//PING(7);
			...
			auto dr = staticDr;
		}else { auto dr = [new Drawing, new Drawing]; }
		
				//init clipbounds
				foreach(i, ref d;	dr) {
			ref view() { return targetSurfaces[i].view; }
	...
	//void focusExit()	  { focusedState.reset; }
		...
		auto ContainerRef(string name) { return q{ref auto #() { return actContainer.#; }}.replace("#", name); }
		...
		Unqual!T clamp(T)(T f) const {
			if(isComplete) {
				...
				static if(isIntegral!T) {
					if(!isnan(min) && f<min.iceil) f = min.iceil;else if(!isnan(max) && f>max.ifloor)
					f = max.ifloor;
				}else {
					if(!isnan(min) && f<min) f = min;else if(!isnan(max) && f>max)
					f = max;
				}
			}
			return f;
		...
		auto hitTest(bool enabled) { return hitTest(actContainer, enabled); }
		...
		auto getScrollBar(in Id id) { if(auto p = id in infos) return (*p).slider;else return null; }
		...
		*/
		...
		static if(__traits(compiles, new CType)) { auto cntr = new CType; }else {
		...
		void CellRef(Cell cell) { if(cell) Container({ actContainer.append(cell); }); }
	
		void CellRef(Cell[] cells) { if(cells.length) Container({ actContainer.append(cells); }); }
	...
	//string flex(float value){ return flex(value.text); } //kinda lame to do it with texts
		 ...
		 //todo: not multiline yet
				...
				static foreach(a; args) {
			{
				...
				static if(is(t == _FlexValue)) {
					...
					textStyle = a;
				}else static if(is(t == RGB)) { textStyle.fontColor = a; }else static if(is(t == SyntaxKind)) { textStyle.applySyntax(a); }else static if(__traits(compiles, a())) { a(); }else {
					...
					*/
					...
					}else if(.Row row = cast(.Row)actContainer) { row.appendMarkupLine(s, textStyle); }else { actContainer.appendMarkupLine(s, textStyle); }
				}
			}
		}
		...
		*/
		...
		void Bullet(string text) { Bullet({ Text(text); }); }
		...
		float highlight = 0;
		if(!enabled) { style.fontColor = clWinBtnDisabledText; }else {
		...
		: mix(clWinBtn, clWinBtnHoverBorder, hover);
			...
			if(!enabled) { style.fontColor = mix(style.fontColor, style.bkColor, 0.5f); }
				...
				if(focused) {
					...
					with(EditCmd) {
						foreach(ch; mainWindow.inputChars.unTag.byDchar) {
							...
							switch(ch) {
								default:
									if(ch==9 && ch==10) { if(flags.acceptEditorKeys) cmdQueue ~= EditCmd(cInsert, [ch].to!string); }else if(ch>=32) { cmdQueue ~= EditCmd(cInsert, [ch].to!string); }else { unprocessed ~= ch; }
		...
		Row!(srcModule, srcLine)(
			args, {
				...
				}else { if(res.valid && Btn(symbol("Refresh"))) { res.mustRefresh = true; } }
			}
		);
		...
		auto DecBtn(string srcModule=__MODULE__, size_t srcLine=__LINE__, T0, T...)(ref T0 value, T args) { return IncBtn!(srcModule, srcLine, -1)(value, args); }
		...
		auto WhiteBtn(string srcModule=__MODULE__, size_t srcLine=__LINE__, T0, T...)(T0 text, T args) { return Btn!(srcModule, srcLine, true, T0, T)(text, args); }
		...
		Row(
			{
				...
				static if(isSomeString!T0) Text(text);
				else
				text();
				 //delegate
		...
		if(isIntegral!I)
	{
		static customDraw(Drawing dr, .Container cntr) {
			...
			if(!materialStyle) {
				...
				dr.color = clAccent;
				btns.filter!(b => b.flags.selected).each!((b){ with(b.borderBounds) dr.hLine(left, bottom, right); });
			}
		}
		...
		Column!(srcModule, srcLine)(
			{
				...
				if(materialStyle) { margin = "4 0"; }else {
		...
		static foreach(a; args) { { static if(is(typeof(a) == GenericArg!(N, T), string N, T) && N=="includeAll") { includeAll = a; } } }
		...
		TabsPage!(srcModule, srcLine)(
			{
				...
				}else { if(includeAll && idx==len) foreach(a; r) a.unaryFun!mapUI; }
			}
		);
	}
		...
		if(isSomeString!T0 || __traits(compiles, text()) )
	{
		...
		*/
		...
		auto LedBtn(string srcModule=__MODULE__, size_t srcLine=__LINE__, T, Args...)(bool ledState, RGB ledColor, T caption, in Args args) { return LedBtn!(srcModule, srcLine)({ if(ledColor!=clBlack) { flags.hAlign = HAlign.left; Led(ledState, ledColor); } }, caption, args); }
		...
		Column(
			{
				...
				foreach(i, s; items) {
					...
					}else { auto hit = ListBoxItem(selected, s, genericId(i)); }
		...
		if((isSomeString!T0 || __traits(compiles, text())) && Args.length>=1 && __traits(compiles, args[$-1]()) )
	{
		...
		if(popupVisible) { Popup(btn, { Column({ args[$-1](); }); }); }
		return popupVisible;
		...
		if(isFocused(hit.id) && comboState) {
			Popup(
				btn, {
					...
					}else { res = ListBox!(srcModule, srcLine)(idx, items, genericId(1), &inheritComboWidth); }
		...
		auto ComboBox(string srcModule=__MODULE__, size_t srcLine=__LINE__, A, Args...)(ref int idx, in A[] items, Args args) { return ComboBox_idx!(srcModule, srcLine, A, Args)(idx, items, args); }
	
		auto ComboBox(string srcModule=__MODULE__, size_t srcLine=__LINE__, A, Args...)(ref A value, in A[] items, Args args) { return ComboBox_ref!(srcModule, srcLine, A, Args)(value, items, args); }
		...
		void mouseAdjust(ref float nPos, in vec2 mousePos, in range range_, ref int wrapCnt, float adjustSpeed) { mouseAdjust(nPos, mousePos, range_.isClamped, range_.isCircular, range_.isEndless, wrapCnt, adjustSpeed); }
		...
		bool handleMouse(in Id id, in HitInfo hit, ref float nPos, in vec2 mousePos, in range range_, ref int wrapCnt) {
			...
			if(hit.pressed && enabled) {
				...
				if(doJump) { jumpToPoint(nPos, mousePos, range_.isEndless); }
			...
			if(hit.released) {
				...
				if(isLinear(drawn_orientation)) { slowMouse(false); }else { mouseUnlock; }
			}
		...
		override bounds2 getHitBounds() { return outerBounds; }
		...
		if(isFloatingPoint!V || isIntegral!V)
	{
		...
		void AdvancedSliderChkBox(Property p, Property pBool, string capt="") { AdvancedSlider(p, { ChkBox(pBool, capt); }); }
		...
		auto Node(string srcModule=__MODULE__, size_t srcLine=__LINE__, Args...)(ref bool state, string title, void delegate() contents, Args args) { return Node!(srcModule, srcLine)(state, { Text(title); }, contents, args); }
		...
		Flex;
		if(onRefresh !is null) { if(ToolBtn(symbol("Refresh"), enable(!refreshing))) onRefresh(); }
	}
...
struct FieldProps {
	...
	static string makeFullName(string parentFullName, string fieldName) { return [parentFullName, fieldName].filter!(not!empty).join('.'); }
		...
		*/
		...
		static if(isSomeString!T) {
			Row(
				{
					Text(thisFieldProps.getCaption, "\t");
					if(thisFieldProps.choices.length) { ComboBox(data, thisFieldProps.choices, genericId(thisFieldProps.hash), hint(thisFieldProps.hint), enable(!thisFieldProps.isReadOnly), { width = fh*10; }); }else { Edit(data, genericId(thisFieldProps.hash), hint(thisFieldProps.hint), enable(!thisFieldProps.isReadOnly), { width = fh*10; }); }
				}
			);
		...
		}else { static assert(0 ,"Unhandle type: "~T.stringof); }
	}
}
...
void stdUI(Property prop, string parentFullName="") {
	...
	if(auto p = cast(IntProperty)prop) {
		...
		doit(p.act);
	}else if(auto p = cast(BoolProperty)prop) { doit(p.act); }else if(auto p = cast(PropertySet)prop) { stdStructFrame(fp.getCaption, { p.properties.each!stdUI; }); }
}
			...
			Btn(
				{
					...
					*/
					...
					with(dr) {
						...
						dr.lineWidth = 2;
						foreach(d; data) { color = d.color;  hGraph(0, h, d.values, sx, sy); }
					}
					addOverlayDrawing(dr);
				}, genericId(name)
			);
