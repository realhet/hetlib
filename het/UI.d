...
struct im{ static:
  ...
  struct FocusedState{
    ...
    int[] subCellStrOfs;  //string offsets for each cell. Generated by appendMarkupLine!true() Implicitely starts with [0].
...
/*    auto r = new .Row("", textStyle);
  ...
  auto Edit(string file=__FILE__, int line=__LINE__, T0, T...)(ref T0 value, T args){ // Edit /////////////////////////////////
    ...
    Row({
      auto row = cast(.Row)actContainer; //this row
      ...
      if(focused){
        row.appendMarkupLine(s, textStyle, focusedState.subCellStrOfs);
        focusedState.subCellStrOfs.writeln;
      }else{
