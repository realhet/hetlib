...
class View2D {
	...
	public:
		...
		auto subScreenBounds_anim() const {
		 //note: subsceen is the mostly visible area on the view's surface. The portion of the screen that remains visible aster the overlayed GUI.
		return bounds2(
		...
		bounds2 visibleArea(bool animated = true)
	{
		...
		float calcOfs(float s0, float s1, float t0, float t1) { return s0>t0 ? s0-t0 : s1<t1 ? s1-t1 : 0; }
		auto ofs = vec2(
		...
		void zoomAround(in vec2 screenPoint, float amount) {
		  //todo: the zoom and the translation amount is not proportional. Fast zooming to the side looks bad. Zoom in center is ok.
		if(!amount) return;
	...
	*/
		...
		@property string config() { return format("%f %f %f", logScale, origin.x, origin.y); }
...
struct RectClamper {
	...
	vec2 clamp(vec2 p) const { return clamp_noCenter(p) + center; }
