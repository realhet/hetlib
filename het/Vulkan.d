...
version(/+$DIDE_REGION Vulkan classes+/all)
{
	version(/+$DIDE_REGION+/all)
	{
		...
		class VulkanSurface
		{
			...
			auto findGraphicsQueueFamily(VulkanPhysicalDevice physicalDevice)
			{
				foreach(qf; physicalDevice.queueFamilies)
				if(qf.queueCount>0 && qf.queueFlags.GRAPHICS_BIT)
				{
			...
			auto requireGraphicsQueueFamily(VulkanPhysicalDevice physicalDevice)
			{
				return 	findGraphicsQueueFamily(physicalDevice)
					.enforce("No graphics and queue family found for surface."); 
			} 
			
			int findGraphicsQueueFamilyIndex(VulkanPhysicalDevice physicalDevice)
			{
				auto qf = findGraphicsQueueFamily(physicalDevice); 
