...
private auto vectorAccess(int idx, T)(in T a){
  static if(isVector!T) return a.array[idx];
                   else return a;
}
...
if(N.inRange(2, 4)){
  ...
  this(A...)(in A args){
    ...
    static if(args.length==1 && __traits(compiles, setAll(args[0]))){
      ...
      setAll(args[0]);
    }else static if(args.length==1 && isVector!(A[0]) && A[0].array.length>=length){
  ...
  bool isNaN() const { static if(is(CT==bool) || isIntegral!CT) return false; else { import std.math; return std.math.isNaN(array[0]); } }
  ...
  auto opBinary(string op, T)(in T other) const{
    static if(op=="~"){ //concat
      ...
      static if(isNumeric!T){
        Vector!(CommonType!(ComponentType, T), length) res;
        static foreach(i; 0..length) res.array[i] = mixin(format!"array[%d] %s other"(i, op));
      }else static if(isVector!T && T.length==length){
        Vector!(CommonType!(ComponentType, T.ComponentType), length) res;
        static foreach(i; 0..length) res.array[i] = mixin(format!"array[%d] %s other[%s]"(i, op, i));
  ...
  auto opBinaryRight(string op, T)(in T other) const{
    static if(op=="~"){ //concat
      return other ~ [this]; //todo: this isn't working
    }else{
        static if(isNumeric!T){
        Vector!(CommonType!(ComponentType, T), length) res;
        static foreach(i; 0..length) res.array[i] = mixin(format!"other %s array[%d]"(op, i));
      }else{
        static assert(false, "invalid operation");
      }
      return res;
...
if(N.inRange(2, 4) && M.inRange(2, 4)){
  ...
  private void construct(int i, T, Tail...)(T head, Tail tail) { // this is from gl3n library
    static if(i >= M*N) {
      ...
      static foreach(j; 0..head.length) //just inject the vector
        columns[(i+j) / M][(i+j) % M] = head.array[j].to!CT;
  ...
  this(Args...)(in Args args){
    ...
    void setIdentity(T)(in T value){
      static foreach(i; 0 .. min(N, M))
        columns[i][i] = value.to!CT;
    }
    ...
    static if(Args.length==1 && isNumeric!(Args[0])){ // Identity matrix
      ...
      static foreach(i; 0..N)
        static foreach(j; 0..M)
          static if(i<N2 && j<M2) columns[i][j] = args[0].columns[i][j].to!CT;
                             else columns[i][j] = (i==j ? 1 : 0).to!CT;
    }else{
      construct!0(args);
    }
  }
  ...
  auto opBinary(string op, T)(in T other) const{
    static if(isNumeric!T){
      ...
      static foreach(i; 0..N) static foreach(j; 0..M) res[i][j] = mixin(format!"this.columns[%d][%d] %s other"(i, j, op));
      ...
      static if(op=="*"){
        ...
        static foreach(j; 0..T.width) static foreach(i; 0..height){{
          ...
          static foreach(k; 0..T.height)
            sum += columns[k][i] * other.columns[j][k];
          res[j][i] = sum;
        }}
      }else{
        ...
        static foreach(i; 0..N) static foreach(j; 0..M) res[i][j] = mixin(format!"this.columns[%d][%d] %s other.columns[%d][%d]"(i, j, op, i, j));
      }
  ...
  auto opBinaryRight(string op, T)(in T other) const{
    static if(isNumeric!T){
      Matrix!(CommonType!(ComponentType, T), N, M) res;
      static foreach(i; 0..N) static foreach(j; 0..M) res[i][j] = mixin(format!"other %s this.columns[%d][%d]"(op, i, j));
      return res;
    }else{
      static assert(false, "invalid operation");
    }
...
auto clamp(T1, T2, T3)(in T1 val, in T2 lower, in T3 upper){
  static if(anyVector!(AliasSeq!(T1, T2, T3))){
...
auto cmp(A, B)(in A a, in B b){
  static if(isVector!A || isVector!B){
  }else{
    return std.algoritm.cmp(a, b);
  }
}

auto mix(A, B, T)(in A a, in B b, in T t){
  alias Types = AliasSeq!(A, B, T);
  static if(anyVector!Types){
    CommonVectorType!Types res;
    static foreach(i; 0..res.length) res.array[i] = mix(vectorAccess!i(a), vectorAccess!i(b), vectorAccess!i(t));
    return res;
  }else{
    return a*(1-t) + b*t;
  }
