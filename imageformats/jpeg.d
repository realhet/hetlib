...
void read_markers(ref JPEG_Decoder dc)
{
	...
	while(!has_next_scan_header && !dc.eoi_reached)
	{
		...
		debug(DebugJPEG)
		writefln("marker: %s (%1$x)\t", cast(Marker) marker[1]);
		switch(marker[1])
		with(Marker)
		{
			...
			writefln("-> skipping segment");
				...
				break;
			default: throw new ImageIOException("invalid / unsupported marker: " ~ marker[1].text);
		}
...
void derive_mincode_maxcode_valptr(
	...
	in ref short[256] codes, in ref ubyte[16] num_values
) pure
{
	mincode[] = -1;
	maxcode[] = -1;
...
void read_quantization_tables(ref JPEG_Decoder dc)
{
	...
	while(0 < len)
	{
		...
		if(
			3 < table_slot || precision != 0//only 8 bit for baseline
		)
		throw new ImageIOException("invalid / not supported");
...
void read_frame_header(ref JPEG_Decoder dc)
{
	...
	foreach(i; 0..dc.num_comps)
	{
		...
		debug(DebugJPEG)
		writefln(
			"%d comp %d sfx/sfy: %d/%d", i, dc.comps[i].id,
																				 dc.comps[i].sfx,
																				 dc.comps[i].sfy
		);
...
void read_restart_interval(ref JPEG_Decoder dc)
{
	...
	debug(DebugJPEG)
	writeln("restart interval set to: ", dc.restart_interval);
}
...
short[64] decode_block(
	ref JPEG_Decoder dc, ref JPEG_Decoder.Component comp,
...
ubyte[] reconstruct(in ref JPEG_Decoder dc)
{
	...
	switch(dc.num_comps * 10 + dc.tgt_chans)
	{
		case 34, 33:
			...
			if(
			(dc.comps[0].sfx <= 2 && dc.comps[0].sfy <= 2)
		...
		)
		{
			...
			foreach(j; 0 .. dc.height)
			{
				...
				size_t cl1 = si * cs;
				resample(
					dc.comps[1].data[cl0 .. cl0 + dc.comps[1].x],
												 dc.comps[1].data[cl1 .. cl1 + dc.comps[1].x],
												 comp1[]
				);
				resample(
					dc.comps[2].data[cl0 .. cl0 + dc.comps[2].x],
...

pure void STBI__IDCT_1D(
	ref int t0, ref int t1, ref int t2, ref int t3,
