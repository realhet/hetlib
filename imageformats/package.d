...
void read_image_info(in char[] file, out int w, out int h, out int chans) {
	auto reader = scoped!FileReader(file);
	try { return read_png_info(reader, w, h, chans); }catch(Throwable) { reader.seek(0, SEEK_SET); }
	try { return read_jpeg_info(reader, w, h, chans); }catch(Throwable) { reader.seek(0, SEEK_SET); }
	try { return read_bmp_info(reader, w, h, chans); }catch(Throwable) { reader.seek(0, SEEK_SET); }
	try { return read_tga_info(reader, w, h, chans); }catch(Throwable) { reader.seek(0, SEEK_SET); }
...
void copy_line(T)(in T[] src, T[] tgt) pure nothrow { tgt[0..$] = src[0..$]; }
...
package class FileReader : Reader {
	this(in char[] filename) { this(File(filename.idup, "rb")); }
...
package class MemReader : Reader {
	this(in ubyte[] source) { this.source = source; }
...
package class FileWriter : Writer {
	this(in char[] filename) { this(File(filename.idup, "wb")); }
